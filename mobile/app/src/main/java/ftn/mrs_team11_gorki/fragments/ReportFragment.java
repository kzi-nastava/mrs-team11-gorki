...
public class ReportFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        btnGenerate.setOnClickListener(view -> generate());
    ...
    private void pickDate(boolean isFrom) {
        ...
        DatePickerDialog dlg = new DatePickerDialog(requireContext(), (dp, year, month, day) -> {
            ...
            if (isFrom) {
                fromDate = picked;
                etFrom.setText(picked.toString()); // yyyy-MM-dd
            } else {
                toDate = picked;
    ...
    private void generate() {
        ...
        String role = tokenStorage.getRole();

        if (userId == null) {
            setError("User is not logged in (missing userId).");
            return;
        }
        ...
        if ("DRIVER".equals(role) || "ROLE_DRIVER".equals(role)) {
            call = reportService.generateDriverReport(userId, from, to);
        } else {
            call = reportService.generatePassengerReport(userId, from, to);
        }
        ...
        call.enqueue(new Callback<ReportDTO>() {
            ...
            public void onResponse(@NonNull Call<ReportDTO> call, @NonNull Response<ReportDTO> response) {
                ...
                ReportDTO dto = response.body();
                bindSummary(dto);
                bindCharts(dto);

                Toast.makeText(requireContext(), "Report loaded.", Toast.LENGTH_SHORT).show();
            }
